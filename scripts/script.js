...
// нормализует название города перед генерацией запроса
function normalizeCityName(value) {
  return value
    .trim()
    .toLocaleLowerCase("ru-RU")
    .replace(/(^|[\s-])([а-яё])/giu, (_, separator, letter) => {
...
function serializeFormData(form) {
  ...
  return {
    city: normalizeCityName(city.value), // -> возвращаем название города с большой буквы
...
function generateFilterRequest(endpoint, city, timeArray) {
  if (city) {
    endpoint += `filters[city][$containsi]=${encodeURIComponent(city)}&`; // -> добавили encodeURIComponent
  }
  // сделаем через .length
  if (timeArray.length) {
    timeArray.forEach((timeslot) => {
      endpoint += `filters[time_of_day][$eqi]=${encodeURIComponent(timeslot)}&`; // -> добавили encodeURIComponent
    });
  }
